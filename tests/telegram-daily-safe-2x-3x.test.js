...
test('the actual daily runner sends only SAFE, 2x and 3x, all with 90% legs', async () => {
  ...
  assert.deepEqual(Array.from(result.plans, p => p.minProbability), [90,90,90]);
  ...
  for (const ticket of result.results) {
    ...
    assert.ok(ticket.minimumProbability === undefined || ticket.minimumProbability >= 90);
    assert.ok(ticket.combinedOdds >= (ticket.targetOdds === '2' ? 2 : ticket.targetOdds === '3' ? 3 : 1.3));
  }
});

test('the daily runner rejects an entirely below-90% candidate pool', async () => {
  const runner = mockRunner(testCandidates(89));
  const result = await runner.run();
  assert.equal(runner.bookings.length, 0);
  assert.deepEqual(Array.from(result.results, p => p.targetOdds), ['1.30–5.00 SAFE','2','3']);
  assert.ok(result.results.every(p => /No selections met the 90%/.test(p.error)));
});
