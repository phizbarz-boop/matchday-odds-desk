...
const OUT_DIR = path.join(process.cwd(), 'volleyball-discovery-v1');
...
function getSport(o) {
  return normalizeText(o.sport?.name ?? o.sportName ?? o.sport ?? 'Volleyball');
}
...
async function publishSnapshot(events) {
  if (!JOB_SECRET) {
    console.log('[Volleyball V1] TELEGRAM_JOB_SECRET not set; snapshot publish skipped');
    return null;
  }
  const ctrl=new AbortController();
  const t=setTimeout(()=>ctrl.abort(),60000);
  try{
    const res=await fetch(`${MATCHDAY_BASE_URL}/api/internal/volleyball/snapshot`,{
      method:'POST',
...
function parseVolleyballPageFixtureMetadata(bodyText) {
...
(async () => {
  if (!LOGIN_ID || !PASSWORD) {
    console.error('[Volleyball V1] Missing SPORTYSOCIAL_LOGIN_ID or SPORTYSOCIAL_PASSWORD');
    process.exit(1);
  }
  ...
  page.on('response', async res => {
    ...
    try {
      ...
      console.log(`[Volleyball V1] Captured fixture endpoint: ${res.status()} ${url}`);
    } catch (e) {
      console.log(`[Volleyball V1] Fixture endpoint captured but JSON parse failed: ${e.message}`);
    }
  });

  try {
    console.log('[Volleyball V1] Opening SportyBet');
    ...
    for (let round = 0; round < 30; round++) {
      ...
      const parsed = parseVolleyballPageFixtureMetadata(bodyText);
      for (const [gameId, row] of parsed.entries()) {
        pageFixtureMetadata.set(gameId, row);
      }

      console.log(`[Volleyball V1] Metadata scan ${round + 1}: ${pageFixtureMetadata.size} unique fixture rows`);
    ...
    const finalParsed = parseVolleyballPageFixtureMetadata(finalBodyText);
    for (const [gameId, row] of finalParsed.entries()) {
      pageFixtureMetadata.set(gameId, row);
    }

    console.log(`[Volleyball V1] Total page fixture metadata rows parsed: ${pageFixtureMetadata.size}`);

    await page.screenshot({ path: path.join(OUT_DIR, 'volleyball-v1-page.png'), fullPage: true });

    safeJsonWrite('volleyball-page-fixture-metadata.json', Array.from(pageFixtureMetadata.values()));
    ...
    safeJsonWrite('volleyball-events-extracted.json', eventCandidates);

    const publishResult = await publishSnapshot(eventCandidates);
    if (publishResult) safeJsonWrite('volleyball-publish-result.json', publishResult);
    ...
    console.log('[Volleyball V1] SUCCESS');
  } catch (err) {
    console.error('[Volleyball V1] FAILED:', err.message);
    try {
