...
const OUT_DIR = path.join(process.cwd(), 'tennis-discovery-v4');
...
function getSport(o) {
  return normalizeText(o.sport?.name ?? o.sportName ?? o.sport ?? 'Tennis');
}
...
async function publishSnapshot(events) {
  if (!JOB_SECRET) {
    console.log('[Tennis V4] TELEGRAM_JOB_SECRET not set; snapshot publish skipped');
    return null;
  }
  ...
  try{
    ...
    console.log(`[Tennis V4] Snapshot published: events=${payload.events??events.length}, winnerRows=${payload.winnerRows??'?'}, totalGamesRows=${payload.totalRows??'?'}`);
    return payload;
  } finally {clearTimeout(t);}
}

function parseTennisPageFixtureMetadata(bodyText) {
...
(async () => {
  if (!LOGIN_ID || !PASSWORD) {
    console.error('[Tennis V4] Missing SPORTYSOCIAL_LOGIN_ID or SPORTYSOCIAL_PASSWORD');
    process.exit(1);
  }
  ...
  page.on('response', async res => {
    ...
    try {
      ...
      console.log(`[Tennis V4] Captured fixture endpoint: ${res.status()} ${url}`);
    } catch (e) {
      console.log(`[Tennis V4] Fixture endpoint captured but JSON parse failed: ${e.message}`);
    }
  });

  try {
    console.log('[Tennis V4] Opening SportyBet');
    ...
    console.log('[Tennis V4] Login attempt completed');

    console.log('[Tennis V4] Opening Tennis prematch page');
    ...
    const maxMetadataScans = Math.max(30, parseInt(process.env.SPORTYBET_TENNIS_MAX_SCANS || '80', 10));
    const stableScanLimit = Math.max(3, parseInt(process.env.SPORTYBET_TENNIS_STABLE_SCANS || '5', 10));
    let stableRounds = 0;
    let previousCount = 0;

    for (let round = 0; round < maxMetadataScans; round++) {
      const bodyText = await page.locator('body').innerText().catch(() => '');
      const parsed = parseTennisPageFixtureMetadata(bodyText);
      for (const [gameId, row] of parsed.entries()) {
        pageFixtureMetadata.set(gameId, row);
      }

      console.log(`[Tennis V4] Metadata scan ${round + 1}: ${pageFixtureMetadata.size} unique fixture rows`);
      ...
      // Dynamic stop: only stop after several consecutive scans with no growth.
      // This removes the old fixed 30-scan ceiling while avoiding wasteful scrolling.
      if (stableRounds >= stableScanLimit) {
        console.log(`[Tennis V4] Metadata stabilized at ${pageFixtureMetadata.size} rows after ${stableRounds} no-growth scans`);
        break;
      }
    ...
    const finalParsed = parseTennisPageFixtureMetadata(finalBodyText);
    for (const [gameId, row] of finalParsed.entries()) {
      pageFixtureMetadata.set(gameId, row);
    }

    console.log(`[Tennis V4] Total page fixture metadata rows parsed: ${pageFixtureMetadata.size}`);

    await page.screenshot({ path: path.join(OUT_DIR, 'tennis-v4-page.png'), fullPage: true });

    safeJsonWrite('tennis-page-fixture-metadata.json', Array.from(pageFixtureMetadata.values()));
    ...
    safeJsonWrite('tennis-events-extracted.json', eventCandidates);

    const publishResult = await publishSnapshot(eventCandidates);
    if (publishResult) safeJsonWrite('tennis-publish-result.json', publishResult);
    ...
    console.log(`[Tennis V4] Fixture endpoint responses captured: ${captured.length}`);
    console.log(`[Tennis V4] Tennis fixture candidates extracted: ${eventCandidates.length}`);
    console.log(`[Tennis V4] With kickoff: ${withKickoff}/${eventCandidates.length}`);
    console.log(`[Tennis V4] With tournament: ${withTournament}/${eventCandidates.length}`);
    console.log(`[Tennis V4] Complete metadata: ${complete}/${eventCandidates.length}`);
    ...
    console.log('[Tennis V4] SUCCESS');
  } catch (err) {
    console.error('[Tennis V4] FAILED:', err.message);
    try {
