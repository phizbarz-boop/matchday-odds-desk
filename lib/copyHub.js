...
const BOOKING_CODE_DENY = new Set([
  'PLEASE','READY','HERE','FROM','TODAY','TOMORROW','SPORTY','SPORTYBET','BOOKING','TICKET',
  'BETSLIP','CASHOUT','FOLLOW','WINNER','WINNING','BOOM','CLICK','LINK','BIO','CODE','CODES'
]);
function isPlausibleStoredBookingCode(entryOrCode, sourceOverride = null) {
  const entry = entryOrCode && typeof entryOrCode === 'object' ? entryOrCode : null;
  const code = String(entry ? entry.bookingCode : entryOrCode || '').trim().toUpperCase();
  const source = normalizeSource(sourceOverride || entry?.source);
  if (!/^[A-Z0-9_-]{4,24}$/.test(code)) return false;
  if (BOOKING_CODE_DENY.has(code)) return false;
  if (/^CORNER\d+$/i.test(code)) return false;

  // SportySocial supplies the shareCode directly from SportyBet, so it is trusted even
  // in the rare case a valid generated code happens to be letters-only.
  if (source === 'sportysocial') return true;

  // For text-discovered sources, require the shapes we actually validate: mixed
  // alphanumeric 6-10 chars or explicit numeric booking codes 6-12 digits. This
  // prevents historical English-word false positives from ever reaching settlement.
  if (/^(?=.{6,10}$)(?=.*[A-Z])(?=.*\d)[A-Z0-9]+$/.test(code)) return true;
  if (/^\d{6,12}$/.test(code)) return true;
  return false;
}
function reconcileInvalidEntries(store) {
  store.entries = Array.isArray(store.entries) ? store.entries : [];
  let quarantined = 0;
  for (const row of store.entries) {
    if (!row || typeof row !== 'object') continue;
    if (row.quarantined === true) continue;
    if (isPlausibleStoredBookingCode(row)) continue;
    row.quarantined = true;
    row.quarantinedAt = row.quarantinedAt || nowIso();
    row.quarantineReason = row.quarantineReason || 'implausible-booking-code-pattern';
    row.rankingEligible = false;
    row.ownershipRankingEligible = false;
...
function reconcileCodeOwnership(store) {
  ...
  for (const [code, rows] of groups) {
    const candidates = rows.filter(x => !x.quarantined).sort(compareOwnershipCandidates);
    ...
    for (const row of rows) {
      ...
      row.ownershipRankingEligible = row.ownershipStatus === 'original' && !row.quarantined;
...
async function readStore(redis) {
  if (!redis) {
    const store = reconcilePunterIdentities(JSON.parse(JSON.stringify(memory)));
    reconcileInvalidEntries(store);
    return reconcileCodeOwnership(store);
  }
  const raw = await redis.get(COPY_HUB_KEY);
  if (!raw) {
    const store = reconcilePunterIdentities({ punters: {}, entries: [], scannedSourceIds: [], codeOwners: {} });
    reconcileInvalidEntries(store);
    return reconcileCodeOwnership(store);
  }
  try {
    const x = JSON.parse(raw);
    const store = reconcilePunterIdentities({
...
async function addObservedCode(redis, { punter, bookingCode, booking, sourcePostId, sourceUrl, publishedAt, sourceText }) {
  ...
  if (!isPlausibleStoredBookingCode(code, p.source)) throw new Error('implausible booking code');
...
function sanitizeSportySocialCode(code = {}) {
  ...
  const out = {
    ...
    shareCodeDetail: Array.isArray(code.shareCodeDetail) ? code.shareCodeDetail.slice(0, 50).map(sanitizeSportySocialSelection) : [],
  };
  return out;
}
...
function buildLeaderboard(store, { days = 30, limit = 50, source = 'all', settledOnly = false } = {}) {
  ...
  const rows = [...grouped.values()].map(s => {
    ...
    return row;
  }).filter(row => !settledOnly || row.settled > 0)
...
async function settlePending({ redis, getBooking, evaluateBooking, maxChecks = 20 }) {
  let quarantinedFound = 0;
  await mutateStore(redis, async s => { quarantinedFound = reconcileInvalidEntries(s); });
  const store = await readStore(redis);
  const byCode = new Map();
  for (const entry of (store.entries || []).filter(x => x.status === 'pending' && !x.quarantined && isPlausibleStoredBookingCode(x))) {
...
module.exports = {
  ...
  isPlausibleStoredBookingCode,
  reconcileInvalidEntries,
  scanXRecent,
  settlePending,
  getPunterProfile,
};
