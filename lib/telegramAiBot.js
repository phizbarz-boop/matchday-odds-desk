...
function freshUser(id,from={}){return {telegramId:String(id),username:from.username||'',firstName:from.first_name||'',plan:'free',planExpiresAt:null,usageDay:dayKey(),ticketsUsed:0,analyzesUsed:0,llmMessagesUsed:0,preferences:{sport:'football',maxMatchOdds:null,minProbability:70,builder:defaultBuilder(),analyzer:defaultAnalyzer()},createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()}}
function normalizeUser(u,id,from={}){
  ...
  if(out.usageDay!==dayKey()){out.usageDay=dayKey();out.ticketsUsed=0;out.analyzesUsed=0;out.llmMessagesUsed=0}
...
function plansText(){return ['💎 MATCHDAY AI PLANS','','🆓 FREE — ₦0','• 2 AI tickets/day','• Football only','• Up to 10x target','• Max 12 selections','• Bet types: Home / Draw / Away only','• No code analyzer / Copy Hub','','⭐ PRO — ₦5,000/month','• 5 AI tickets/day','• Football + Basketball only','• Up to 100x target','• Max 30 selections','• Expanded bet types: Corners, Double Chance, DNB, goals, GG/NG, Asian Handicap, Basketball','• 2 SportyBet code analyses/day','• Saved preferences','','👑 ELITE — ₦20,000/month','• 10 AI tickets/day','• All supported sports','• Up to 1000x target','• Max 40 selections','• ALL bet types including Corners Over/Under','• 5 code analyses/day','• Copy Hub / punter rankings','• Highest access limits','','All model probabilities are estimates, not guarantees.'].join('\n')}
