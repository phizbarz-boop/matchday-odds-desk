...
async function getRecentCornerProfile(teamId, last=8) {
  ...
  const list=await apiFetch('/fixtures',{team:teamId,last:n,status:'FT-AET-PEN'});
  const fixtures=Array.isArray(list.response)?list.response:[];
  if(!fixtures.length)return null;
  const ids=fixtures.map(x=>x?.fixture?.id).filter(Boolean).slice(0,20);
  let detailed=[];
  if(ids.length){
    try { const batch=await apiFetch('/fixtures',{ids:ids.join('-')}); detailed=Array.isArray(batch.response)?batch.response:[]; } catch {}
  }
  const byId=new Map(detailed.map(x=>[String(x?.fixture?.id),x]));
  const samples=[];
  const fallbackLimit=Math.max(0,Math.min(10,parseInt(process.env.API_FOOTBALL_CORNER_STAT_FALLBACK_CALLS||'6',10)));
  let fallbacks=0;
...
async function enrichSportyFixtures(sportyEvents,{daysAhead=7,maxFixtures=180,cornerEventIds=new Set()}={}) {
  ...
  for(const m of limited){
    ...
    const row=pred?modelFromApiPrediction(m.api,pred):null; if(!row)continue;
    // Preserve the exact SportyBet names for cross-source matching in the app.
    row.home=m.sporty.home; row.away=m.sporty.away; row.league=m.sporty.tournament||row.league; row.kickoffUtc=m.sporty.kickoffUtc||row.kickoffUtc;
    row.apiFootballMatchConfidence=Math.round(m.matchConfidence*100);
    if(cornerEventIds.has(String(m.sporty.eventId))){
