...
function modelFromApiPrediction(apiFixture, pred) {
  ...
  let btts=0,o05=0,o15=0,u45=0,o25=0,oneUpHome=0,oneUpAway=0,mass=0,top={h:0,a:0,p:0};
  for(let hg=0;hg<matrix.length;hg++)for(let ag=0;ag<matrix[hg].length;ag++){
    ...
    if(hg>0&&ag>0)btts+=pr; if(hg+ag>0)o05+=pr; if(hg+ag>1)o15+=pr; if(hg+ag>2)o25+=pr; if(hg+ag<5)u45+=pr;
    oneUpHome+=pr*(hg>ag?1:hg/(ag+1)); oneUpAway+=pr*(ag>hg?1:ag/(hg+1));
  }
  if(mass>0){btts/=mass;o05/=mass;o15/=mass;o25/=mass;u45/=mass;oneUpHome/=mass;oneUpAway/=mass;}
  let hh=h,dd=d,aa=a;
  ...
  return {
    ...
    h:Math.round(hh), d:Math.round(dd), a:Math.round(aa), btts:Math.round(btts*100), o05:Math.round(o05*100), o15:Math.round(o15*100),
...
function cornerModel(homeProfile,awayProfile,historical=null) {
  if(!homeProfile||!awayProfile)return null;
  const recentHome=(homeProfile.cornersFor+awayProfile.cornersAgainst)/2;
  const recentAway=(awayProfile.cornersFor+homeProfile.cornersAgainst)/2;
  const histHome=Number(historical?.homeExpected);
  const histAway=Number(historical?.awayExpected);
  const histSamples=Math.min(Number(historical?.samplesHome||0),Number(historical?.samplesAway||0));
  const hasHistory=histSamples>=8 && histHome>0 && histAway>0;

  // Historical season/venue/opponent data gets the larger share once a meaningful
  // sample exists. Recent form still matters so the model adapts to tactical changes.
  const historicalWeight=hasHistory?Math.max(0.45,Math.min(0.70,Number(process.env.API_FOOTBALL_CORNER_HISTORY_WEIGHT||0.55))):0;
  const recentWeight=1-historicalWeight;
  let homeLambda=recentHome*recentWeight+(hasHistory?histHome*historicalWeight:0);
  let awayLambda=recentAway*recentWeight+(hasHistory?histAway*historicalWeight:0);

  // Add a small league-baseline stabilizer, not enough to erase team-specific signal.
  const leagueAvg=Number(historical?.leagueAvgTotal);
  if(hasHistory && leagueAvg>0){
    const baselineWeight=Math.max(0,Math.min(0.20,Number(process.env.API_FOOTBALL_CORNER_LEAGUE_BASELINE_WEIGHT||0.10)));
    const scale=(homeLambda+awayLambda)>0?leagueAvg/(homeLambda+awayLambda):1;
    homeLambda=homeLambda*(1-baselineWeight)+homeLambda*scale*baselineWeight;
    awayLambda=awayLambda*(1-baselineWeight)+awayLambda*scale*baselineWeight;
  }

  homeLambda=Math.max(0.4,Math.min(11,homeLambda));
  awayLambda=Math.max(0.4,Math.min(11,awayLambda));
  const totalLambda=Math.max(2,Math.min(18,homeLambda+awayLambda));

  // IMPORTANT: no fabricated 46% first-half share. 1H lambdas only exist when
  // actual historical half statistics are cached from API-Football half=true.
  const firstHalfHome=Number(historical?.firstHalfHomeExpected);
  const firstHalfAway=Number(historical?.firstHalfAwayExpected);
  const h1Samples=Math.min(Number(historical?.firstHalfSamplesHome||0),Number(historical?.firstHalfSamplesAway||0));
  const hasRealFirstHalf=h1Samples>=6 && firstHalfHome>=0 && firstHalfAway>=0;
  const recentTotal=recentHome+recentAway;
  const historicalTotal=hasHistory?histHome+histAway:null;
  const disagreementPct=hasHistory&&historicalTotal>0?Math.abs(recentTotal-historicalTotal)/historicalTotal*100:null;
  ...
  return {
    ...
    recentHomeLambda:Math.round(recentHome*100)/100,
    recentAwayLambda:Math.round(recentAway*100)/100,
    recentTotalLambda:Math.round(recentTotal*100)/100,
    historicalHomeLambda:hasHistory?Math.round(histHome*100)/100:null,
    historicalAwayLambda:hasHistory?Math.round(histAway*100)/100:null,
    historicalTotalLambda:hasHistory?Math.round(historicalTotal*100)/100:null,
    historicalWeight:Math.round(historicalWeight*1000)/1000,
    historicalSeason:historical?.season??null,
    historicalLeagueId:historical?.leagueId??null,
    historicalMatchHitRates:historical?.matchHitRates||null,
    historicalHomeTeamHitRates:historical?.homeTeamHitRates||null,
    historicalAwayTeamHitRates:historical?.awayTeamHitRates||null,
    firstHalfHomeLambda:hasRealFirstHalf?Math.round(firstHalfHome*100)/100:null,
    firstHalfAwayLambda:hasRealFirstHalf?Math.round(firstHalfAway*100)/100:null,
    firstHalfTotalLambda:hasRealFirstHalf?Math.round((firstHalfHome+firstHalfAway)*100)/100:null,
    firstHalfHomeHitRates:hasRealFirstHalf?(historical?.firstHalfHomeHitRates||null):null,
    firstHalfAwayHitRates:hasRealFirstHalf?(historical?.firstHalfAwayHitRates||null):null,
    firstHalfShare:hasRealFirstHalf&&totalLambda>0?Math.round(((firstHalfHome+firstHalfAway)/totalLambda)*1000)/1000:null,
    firstHalfDataSource:hasRealFirstHalf?'API-Football historical half=true':null,
    samplesHome:homeProfile.samples,
    samplesAway:awayProfile.samples,
    historicalSamplesHome:Number(historical?.samplesHome||0),
    historicalSamplesAway:Number(historical?.samplesAway||0),
    historicalVenueSamplesHome:Number(historical?.venueSamplesHome||0),
    historicalVenueSamplesAway:Number(historical?.venueSamplesAway||0),
    firstHalfSamplesHome:Number(historical?.firstHalfSamplesHome||0),
    firstHalfSamplesAway:Number(historical?.firstHalfSamplesAway||0),
    recentVsHistoricalDisagreementPct:Number.isFinite(disagreementPct)?Math.round(disagreementPct*10)/10:null,
    modelVersion:'corner-history-v3',
  };
}
...
async function enrichSportyFixtures(sportyEvents,{daysAhead=7,maxFixtures=180,cornerEventIds=new Set(),historyLookup=null}={}) {
  ...
  for(const m of limited){
    ...
    if(buildAllCorners || cornerEventIds.has(String(m.sporty.eventId))){
      ...
      if(hid&&aid){
        const hp=await getCached(hid),ap=await getCached(aid);
        let historical=null;
        if(typeof historyLookup==='function'){
          try{historical=await historyLookup({apiFixture:m.api,homeTeamId:hid,awayTeamId:aid});}
          catch(e){console.warn(`[API-Football] corner history lookup ${m.api?.fixture?.id}: ${e.message}`);}
        }
        const cm=cornerModel(hp,ap,historical);if(cm)row.corners=cm;
      }
    }
    out.push(row);
  }
