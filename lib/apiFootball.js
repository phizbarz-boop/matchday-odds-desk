...
function modelFromApiPrediction(apiFixture, pred) {
  ...
  let btts=0,o05=0,o15=0,u45=0,homeU45=0,awayU45=0,o25=0,oneUpHome=0,oneUpAway=0,mass=0,top={h:0,a:0,p:0};
  for(let hg=0;hg<matrix.length;hg++)for(let ag=0;ag<matrix[hg].length;ag++){
    ...
    if(hg<5)homeU45+=pr; if(ag<5)awayU45+=pr;
  ...
  if(mass>0){btts/=mass;o05/=mass;o15/=mass;o25/=mass;u45/=mass;homeU45/=mass;awayU45/=mass;oneUpHome/=mass;oneUpAway/=mass;}
  let hh=h,dd=d,aa=a;
  ...
  return {
    ...
    u45:Math.round(u45*100), homeU45:Math.round(homeU45*100), awayU45:Math.round(awayU45*100), o25:Math.round(o25*100),
    homeO05:Math.round((1-Math.exp(-lh))*100), awayO05:Math.round((1-Math.exp(-la))*100),
    oneUpHome:Math.round(oneUpHome*100), oneUpAway:Math.round(oneUpAway*100),
...
async function getRecentGoalProfile(teamId, last=6) {
  const n=Math.max(3,Math.min(12,parseInt(last,10)||6));
  const j=await apiFetch('/fixtures',{team:teamId,last:Math.min(24,n*2),timezone:'UTC'});
  const finished=new Set(['FT','AET','PEN']);
  const rows=(Array.isArray(j.response)?j.response:[])
    .filter(x=>finished.has(String(x?.fixture?.status?.short||'')))
    .slice(0,n);
  if(rows.length<3) return null;
  let gf=0,ga=0,tot=0,btts=0,o05=0,o15=0;
  for(const fx of rows){
    const isHome=Number(fx?.teams?.home?.id)===Number(teamId);
    const hg=Number(fx?.goals?.home), ag=Number(fx?.goals?.away);
    if(!Number.isFinite(hg)||!Number.isFinite(ag)) continue;
    const f=isHome?hg:ag, a=isHome?ag:hg;
    gf+=f; ga+=a; tot++;
    if(hg+ag>0)o05++;
    if(hg+ag>1)o15++;
    if(hg>0&&ag>0)btts++;
  ...
  return {
    teamId:Number(teamId), samples:tot,
    goalsFor:gf/tot, goalsAgainst:ga/tot,
    over05Rate:o05/tot, over15Rate:o15/tot, bttsRate:btts/tot,
  };
}

function modelFromRecentGoals(apiFixture, homeProfile, awayProfile) {
  if(!homeProfile||!awayProfile) return null;
  let lh=(Number(homeProfile.goalsFor)+Number(awayProfile.goalsAgainst))/2;
  let la=(Number(awayProfile.goalsFor)+Number(homeProfile.goalsAgainst))/2;
  if(!Number.isFinite(lh)||!Number.isFinite(la)) return null;
  const leaguePrior=1.28;
  lh=0.82*lh+0.18*leaguePrior;
  la=0.82*la+0.18*leaguePrior;
  lh=Math.min(4.5,Math.max(0.18,lh));
  la=Math.min(4.5,Math.max(0.18,la));

  const matrix=scoreMatrix(lh,la);
  let wh=0,wd=0,wa=0,btts=0,o05=0,o15=0,o25=0,u45=0,homeU45=0,awayU45=0,mass=0,top={h:0,a:0,p:0};
  for(let hg=0;hg<matrix.length;hg++)for(let ag=0;ag<matrix[hg].length;ag++){
    const pr=matrix[hg][ag]; mass+=pr;
    if(pr>top.p) top={h:hg,a:ag,p:pr};
    if(hg>ag)wh+=pr; else if(hg===ag)wd+=pr; else wa+=pr;
    if(hg>0&&ag>0)btts+=pr;
    if(hg+ag>0)o05+=pr;
    if(hg+ag>1)o15+=pr;
    if(hg+ag>2)o25+=pr;
    if(hg+ag<5)u45+=pr;
  ...
  wh/=mass; wd/=mass; wa/=mass; btts/=mass; o05/=mass; o15/=mass; o25/=mass; u45/=mass; homeU45/=mass; awayU45/=mass;
  return {
    league:apiFixture?.league?.name||'', leagueCode:`api:${apiFixture?.league?.id||''}`,
    home:apiFixture?.teams?.home?.name||'', away:apiFixture?.teams?.away?.name||'',
    kickoffUtc:apiFixture?.fixture?.date||null, apiFootballFixtureId:apiFixture?.fixture?.id||null,
    apiFootballHomeTeamId:apiFixture?.teams?.home?.id||null, apiFootballAwayTeamId:apiFixture?.teams?.away?.id||null,
    h:Math.round(wh*100), d:Math.round(wd*100), a:Math.round(wa*100),
    btts:Math.round(btts*100), o05:Math.round(o05*100), o15:Math.round(o15*100),
    u45:Math.round(u45*100), homeU45:Math.round(homeU45*100), awayU45:Math.round(awayU45*100), o25:Math.round(o25*100),
    homeO05:Math.round((1-Math.exp(-lh))*100), awayO05:Math.round((1-Math.exp(-la))*100),
    oneUpHome:0, oneUpAway:0,
    score:`${top.h}-${top.a}`, scoreP:Math.round(top.p/mass*100),
    pick:['Home Win','Draw','Away Win'][[wh,wd,wa].indexOf(Math.max(wh,wd,wa))],
    pickProb:Math.round(Math.max(wh,wd,wa)*100),
    dataSource:'API-Football recent-results fallback',
    apiFootballGoalLambdaHome:Math.round(lh*100)/100,
    apiFootballGoalLambdaAway:Math.round(la*100)/100,
    recentGoalSamplesHome:homeProfile.samples,
    recentGoalSamplesAway:awayProfile.samples,
  };
}

async function enrichSportyFixtures(sportyEvents,{daysAhead=7,maxFixtures=500,cornerEventIds=new Set()}={}) {
  ...
  const dateEntries=[...byDate.entries()].sort((a,b)=>a[0].localeCompare(b[0])).slice(0,Math.max(1,daysAhead+1));
  const matched=[];
  ...
  matched.sort((a,b)=>{
    const at=Number(a?.api?.fixture?.timestamp)||0, bt=Number(b?.api?.fixture?.timestamp)||0;
    return at-bt || b.matchConfidence-a.matchConfidence;
  });
  const limited=matched.slice(0,Math.max(1,maxFixtures));
  const teamCornerCache=new Map();
  const teamGoalCache=new Map();
  const goalFallbackLast=Math.max(3,Math.min(12,parseInt(process.env.API_FOOTBALL_GOAL_FALLBACK_LAST_MATCHES||'6',10)));
  const goalFallbackMaxTeams=Math.max(0,Math.min(500,parseInt(process.env.API_FOOTBALL_GOAL_FALLBACK_MAX_TEAMS||'240',10)));
  let goalFallbackTeamCalls=0;
  const out=[];
  for(const m of limited){
    ...
    // Prefer API-Football's prediction payload. For lower leagues where it has the
    // fixture but no usable goal estimate, derive a conservative Poisson model from
    // each team's recent completed scores. No SportyBet/booking calls are added here.
    let row=pred?modelFromApiPrediction(m.api,pred):null;
    if(!row){
      const getGoal=async id=>{
        const k=String(id||'');
        if(!k) return null;
        if(teamGoalCache.has(k)) return teamGoalCache.get(k);
        if(goalFallbackTeamCalls>=goalFallbackMaxTeams){teamGoalCache.set(k,null);return null;}
        goalFallbackTeamCalls++;
        let p=null;
        try{p=await getRecentGoalProfile(id,goalFallbackLast);}
        catch(e){console.warn(`Recent-goal profile team ${id}: ${e.message}`);}
        teamGoalCache.set(k,p);
        return p;
      };
      const hid=m.api?.teams?.home?.id, aid=m.api?.teams?.away?.id;
      if(hid&&aid){
        const hp=await getGoal(hid), ap=await getGoal(aid);
        row=modelFromRecentGoals(m.api,hp,ap);
      }
    }
    if(!row){
      row={
...
module.exports={apiFetch,getFixturesByDate,getPrediction,getRecentGoalProfile,getRecentCornerProfile,cornerModel,totalCornerProbability,enrichSportyFixtures,fixtureMatchScore,normTeam};
