...
function matchOne(sporty, apiGames) {
  ...
  if(!best) return null;
  const pairFloor = Math.min(Number(best.directHome||0), Number(best.directAway||0));
  // Permit a slightly lower composite score only when both team-name sides
  // independently have useful similarity. This recovers spelling/alias cases
  // without accepting a single-team accidental match.
  if(best.score < 0.64 && !(best.score >= 0.59 && pairFloor >= 0.45)) return null;
