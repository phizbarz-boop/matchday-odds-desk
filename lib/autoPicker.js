...
const MARKET_RELIABILITY = {
  ...
  'Football:dnb': 0.90,
  'Football:ou05': 0.91,
  'Football:home_ou05': 0.88,
  'Football:away_ou05': 0.88,
  'Football:home_ou45': 0.88,
  'Football:away_ou45': 0.88,
  'Football:ou15': 0.91,
  'Football:ou45': 0.91,
  'Football:ah': 0.90,
  'Football:oneup': 0.88,
  'Football:corners': 0.84,
  'Football:first_half_team_corners': 0.78,
  'Football:oneup': 0.92,
  'Basketball:winner': 0.86,
  'Basketball:totals': 0.84,
  'Ice Hockey:winner': 0.82,
  'Ice Hockey:totals': 0.83,
  'Handball:winner': 0.82,
  'Handball:totals': 0.83,
  'Volleyball:winner': 0.84,
  'Volleyball:totals': 0.83,
  'Volleyball:sets': 0.82,
  'Tennis:winner': 0.84,
  'Tennis:totals': 0.82,
  'Tennis:handicap': 0.82,
};
...
function teamOver05Candidate(r, rows, side) {
  const kind = side === 'home' ? 'home_ou05' : 'away_ou05';
  const betType = side === 'home' ? 'home_over05' : 'away_over05';
  const stored = side === 'home' ? r.homeO05 : r.awayO05;
  // Older cache entries have no individual team-goal model. Skip until a refresh
  // produces one; never use full-match O0.5 as a team's probability.
  if (stored === null || stored === undefined || !Number.isFinite(Number(stored))) return null;
  const prob = Number(stored);
  const sp = matchingRows(r, rows).find(x => {
    const market = String(x.marketDesc || '').toLowerCase();
    if (!new RegExp(`\\b${side}\\b`).test(market) || /half|corner|period|quarter/.test(market)) return false;
    return (/\bover\b/i.test(String(x.outcomeDesc || '')) || String(x.outcomeId) === '12')
      && (String(x.specifier || '').toLowerCase() === 'total=0.5'
        || (!x.specifier && /0\.5/.test(String(x.outcomeDesc || ''))));
  });
  return sp ? candidate(sp, { sport:'Football', home:r.home, away:r.away,
    tournament:r.league, kickoffUtc:r.kickoffUtc }, normalMetrics(sp.odds, prob),
    'Poisson + H2H · team goals', `Football:${kind}`, betType) : null;
}

function teamUnder45Candidate(r, rows, side) {
  const kind = side === 'home' ? 'home_ou45' : 'away_ou45';
  const betType = side === 'home' ? 'home_under45' : 'away_under45';
  const stored = side === 'home' ? r.homeU45 : r.awayU45;
  // Never reuse the full-match Under 4.5 probability for an individual team.
  // Previous cached models without team-specific values must be refreshed.
  if (stored === null || stored === undefined || !Number.isFinite(Number(stored))) return null;
  const sp = matchingRows(r, rows).find(x => isTeamTotalSelection(x, side, '4.5', 'under'));
  return sp ? candidate(sp, { sport:'Football', home:r.home, away:r.away,
    tournament:r.league, kickoffUtc:r.kickoffUtc }, normalMetrics(sp.odds, Number(stored)),
    'Poisson + H2H · team goals', `Football:${kind}`, betType) : null;
}
...
function parseTotalLine(row) {
  const s = String(row?.specifier || row?.outcomeDesc || '');
  const m = s.match(/(?:total\s*=\s*)?(\d+(?:\.\d+)?)/i);
  return m ? Number(m[1]) : null;
}
...
function cornerCandidates(r, rows) {
  ...
  for (const sp of fx) {
    ...
    // Keep the initial corner integration to normal half-lines, which have no push settlement.
    if (Math.abs((line * 2) - Math.round(line * 2)) > 0.001 || Math.abs(line - Math.floor(line) - 0.5) > 0.001) continue;
    const prob = cornerTotalProbability(lambda, line, isOver);
    ...
    if (c) {
      ...
      c.settlementNote = `Corner model based on API-Football recent match statistics (${r?.corners?.samplesHome || '?'} / ${r?.corners?.samplesAway || '?'} samples)`;
      out.push(c);
    }
  }
  return out;
...
function firstHalfTeamCornerCandidates(r, rows) {
  const homeLambda = Number(r?.corners?.firstHalfHomeLambda || 0);
  const awayLambda = Number(r?.corners?.firstHalfAwayLambda || 0);
  if (!(homeLambda > 0) || !(awayLambda > 0)) return [];
  const fx = matchingRows(r, rows);
  const out = [];
  for (const sp of fx) {
    const desc = `${sp?.marketDesc || ''} ${sp?.outcomeDesc || ''} ${sp?.specifier || ''}`.toLowerCase();
    // Only accept explicitly first-half team-corner markets; never reinterpret a full-match corner row.
    ...
    let side = null;
    if (teamMatches(sp?.outcomeDesc, r.home) || /\bhome\b/.test(desc)) side='home';
    if (teamMatches(sp?.outcomeDesc, r.away) || /\baway\b/.test(desc)) side='away';
    // Some feeds put team in specifier/market text.
    if (!side && teamMatches(desc, r.home)) side='home';
    if (!side && teamMatches(desc, r.away)) side='away';
    if (!side) continue;
    ...
    const c = candidate(sp,
      { sport:'Football', home:r.home, away:r.away, tournament:r.league, kickoffUtc:r.kickoffUtc },
      normalMetrics(sp.odds, prob),
      `API-Football 1H team-corner model · ${side} λ ${lambda.toFixed(2)}`,
      'Football:first_half_team_corners', bt);
    if (c) {
      c.cornerLine=line; c.cornerLambda=lambda; c.cornerPeriod='1H'; c.cornerTeam=side;
      c.outcomeDesc=`1st Half ${side==='home'?r.home:r.away} ${isOver?'Over':'Under'} ${line} Corners`;
      c.settlementNote=`1H team-corner estimate uses recent API-Football corner rates and a first-half share (${Math.round(Number(r?.corners?.firstHalfShare||0.46)*100)}%).`;
      out.push(c);
    }
  }
  return out;
...
function buildCandidates({ predictions, footballMarkets, basketballWinner, basketballTotals, hockeyWinner, hockeyTotals, handballWinner, handballTotals, volleyballWinner, volleyballTotals, volleyballSets, tennisWinner, tennisTotals, tennisHandicap, minProbability = 55, minEdge = 0, leagues = null, sportScope = 'all', betTypes = null }) {
  ...
  const scope = ['all','football','basketball','hockey','handball','volleyball','tennis'].includes(String(sportScope).toLowerCase()) ? String(sportScope).toLowerCase() : 'all';
  const candidates = [];
  ...
  if (scope === 'all' || scope === 'football') {
    for (const r of predictions?.matches || []) {
      ...
      const generated = [
        ...
        teamOver05Candidate(r, footballMarkets?.home_ou05?.rows || [], 'home'),
        teamOver05Candidate(r, footballMarkets?.away_ou05?.rows || [], 'away'),
        teamUnder45Candidate(r, footballMarkets?.home_ou45?.rows || [], 'home'),
        teamUnder45Candidate(r, footballMarkets?.away_ou45?.rows || [], 'away'),
  ...
  for (const [sport,payload,scopeKey] of [['Basketball',basketballWinner,'basketball'],['Ice Hockey',hockeyWinner,'hockey'],['Handball',handballWinner,'handball'],['Volleyball',volleyballWinner,'volleyball'],['Tennis',tennisWinner,'tennis']]) {
    ...
    for (const e of events) {
      ...
      const bt = scopeKey === 'basketball' ? 'basketball_winner' : (scopeKey === 'hockey' ? 'hockey_winner' : (scopeKey === 'handball' ? 'handball_winner' : (scopeKey === 'volleyball' ? 'volleyball_winner' : 'tennis_winner')));
  ...
  // Basketball, Ice Hockey, Handball and Volleyball totals. Each total line (e.g. 214.5 points or 5.5 goals)
  ...
  // the honest probability estimate is the market's margin-removed probability. Handball and Volleyball use this exact same model.
  for (const [sport,payload,scopeKey] of [['Basketball',basketballTotals,'basketball'],['Ice Hockey',hockeyTotals,'hockey'],['Handball',handballTotals,'handball'],['Volleyball',volleyballTotals,'volleyball'],['Tennis',tennisTotals,'tennis']]) {
    if (scope !== 'all' && scope !== scopeKey) continue;
  ...
  // Volleyball Total Sets is kept separate from Total Points so a user can
  // explicitly choose which O/U family the Auto Builder is allowed to use.
  if (scope === 'all' || scope === 'volleyball') {
    const byLine = new Map();
    for (const r of volleyballSets?.rows || []) {
      const key = `${r.eventId}|${r.marketId}|${r.specifier || ''}`;
      if (!byLine.has(key)) byLine.set(key, []);
      byLine.get(key).push(r);
    }
    for (const rows of byLine.values()) {
      if (rows.length < 2) continue;
      const inv = rows.map(r => 1 / Number(r.odds));
      const sum = inv.reduce((acc,v)=>acc+(Number.isFinite(v)?v:0),0);
      if (!sum) continue;
      for (let i=0;i<rows.length;i++) {
        const r=rows[i];
        const prob=round1(inv[i]/sum*100);
        ...
        const isUnder=/\bunder\b/.test(desc) || String(r.outcomeId)==='13';
        if (!isOver && !isUnder) continue;
        const bt=`volleyball_sets_${isOver?'over':'under'}`;
        const metrics=normalMetrics(r.odds,prob);
        const c=candidate(
          r,
          {sport:'Volleyball',home:r.home,away:r.away,tournament:r.tournament,marketDesc:r.marketDesc,outcomeDesc:r.outcomeDesc,kickoffUtc:r.kickoffUtc},
          metrics,
          'No-vig Total Sets market probability',
          'Volleyball:sets',
          bt
        );
        if (c && allowed(c)) candidates.push(c);
  ...
  // Tennis Set Handicap (SportyBet market 188) uses the same no-vig
  // two-way probability model as other non-football markets.
  if (scope === 'all' || scope === 'tennis') {
    const byLine = new Map();
    for (const r of tennisHandicap?.rows || []) {
      const key = `${r.eventId}|${r.marketId}|${r.specifier || ''}`;
      if (!byLine.has(key)) byLine.set(key, []);
      byLine.get(key).push(r);
    }
    for (const rows of byLine.values()) {
      ...
      for (let i=0;i<rows.length;i++) {
        ...
        if (!Number.isFinite(prob) || prob < minProb) continue;
        const desc=String(r.outcomeDesc||'').toLowerCase();
        const isHome=desc.includes('home') || String(r.outcomeId)==='1714';
        const isAway=desc.includes('away') || String(r.outcomeId)==='1715';
        if (!isHome && !isAway) continue;
        const bt=`tennis_handicap_${isHome?'home':'away'}`;
        const metrics=normalMetrics(r.odds,prob);
        const c=candidate(
          r,
          {sport:'Tennis',home:r.home,away:r.away,tournament:r.tournament,marketDesc:r.marketDesc,outcomeDesc:r.outcomeDesc,kickoffUtc:r.kickoffUtc},
          metrics,
          'No-vig Tennis Set Handicap market probability',
          'Tennis:handicap',
          bt
        );
        if (c && allowed(c)) candidates.push(c);
...
function evaluateCombo(combo, targetOdds) {
  ...
  const reachedTarget = combinedOdds + 1e-9 >= targetOdds;
  const avgOdds = combo.reduce((s,x)=>s+Number(x.odds||0),0) / Math.max(1,combo.length);
  // SportyBet allows at most 50 selections. For large targets we intentionally
  // accumulate the target with more high-probability/lower-odds legs rather than
  // optimizing for the fewest legs. Once the target is reached, a slightly larger
  // overshoot is acceptable if it comes from a longer, safer slip.
  const score = reachedTarget
    ? distance*0.10 - (avgQualityScore/100)*0.16 - (avgProbability/100)*0.10 - clamp(avgEdge,-20,20)*0.0025 + Math.max(0,avgOdds-1.01)*0.12 - combo.length*0.015
    : 5 + distance - (avgQualityScore/100)*0.12 - (avgProbability/100)*0.08 - clamp(avgEdge,-20,20)*0.0020 + Math.max(0,avgOdds-1.01)*0.08 - combo.length*0.006;
  return { combinedOdds,avgProbability,minProbability,avgEdge,avgQualityScore,avgOdds,estimatedSlipProbability,fairSlipOdds,estimatedSlipEVPct,score,reachedTarget };
}
...
function weightedOrder(candidates, rng = Math.random) {
  return candidates.map(c => {
    ...
    const lowOddsPreference = 1 / Math.max(1.01, Number(c.odds)||1.01);
    const w = Math.pow(q*0.50+p*0.40+lowOddsPreference*0.10,5);
...
function selectAutoBet(candidates, { targetOdds=5,maxSelections=8,trials=1200,minQualityScore=0,requirePositiveEV=false,rng=Math.random } = {}) {
  ...
  const maxLegs = clamp(parseInt(maxSelections,10)||8,1,50);
  ...
  const orders = [usable.slice().sort((a,b)=>b.probability-a.probability||b.qualityScore-a.qualityScore||a.odds-b.odds)];
