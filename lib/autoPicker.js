...
function buildCandidates({ predictions, footballMarkets, basketballWinner, hockeyWinner, minProbability = 55, leagues = null }) {
  ...
  for (const [sport, payload] of [['Basketball', basketballWinner], ['Ice Hockey', hockeyWinner]]) {
...
function selectAutoBet(candidates, { targetOdds = 5, maxSelections = 8, trials = 1200, rng = Math.random } = {}) {
  const target = clamp(Number(targetOdds) || 5, 1.05, 500);
  const maxLegs = clamp(parseInt(maxSelections, 10) || 8, 1, 20);
