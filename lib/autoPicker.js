...
function selectAutoBet(candidates, { targetOdds=5,maxSelections=8,trials=1200,minQualityScore=0,minEdge=-25,requirePositiveEdge=false,requirePositiveEV=false,applyRedFlagFilter=true,targetMinOdds=null,targetMaxOdds=null,rng=Math.random } = {}) {
  ...
  const usable = (candidates||[]).filter(c =>
    Number.isFinite(c.odds) && c.odds > 1 && c.probability > 0 &&
    c.qualityScore >= qualityFloor &&
    c.edge >= edgeFloor &&
    (!requirePositiveEdge || c.edge > 0) &&
    (!requirePositiveEV || c.expectedValuePct > 0) &&
    (!applyRedFlagFilter || passesRedFlagFilter(c))
  );
  ...
  for (const order of orders) {
    ...
    for (const c of order) {
      ...
      const rangeMin = Number(targetMinOdds);
      const rangeMax = Number(targetMaxOdds);
      const useRange = Number.isFinite(rangeMin) && Number.isFinite(rangeMax) && rangeMax >= rangeMin;
      const inRange = !useRange || (product >= rangeMin && product <= rangeMax);
      const rangePenalty = useRange && !inRange ? 100 + Math.abs(Math.log(Math.max(product,1.000001)/Math.max(rangeMin,1.000001))) : 0;
      const ranked = { ...evalNow, score: evalNow.score + rangePenalty, inTargetRange: inRange };
      if (!best || ranked.score<best.metrics.score) best={selections:combo.slice(),metrics:ranked};
      if (useRange ? product>=rangeMax : product>=target) break;
    }
  }
  if (!best) return { selections:[],targetOdds:target,combinedOdds:1,reachedTarget:false,candidateCount:usable.length };
  const m=best.metrics;
  return {
    selections:best.selections,targetOdds:target,combinedOdds:round2(m.combinedOdds),reachedTarget:(Number.isFinite(Number(targetMinOdds))&&Number.isFinite(Number(targetMaxOdds))?m.inTargetRange:m.reachedTarget),
...
module.exports = { buildCandidates, selectAutoBet, selectionRedFlags, passesRedFlagFilter, groupMarkets, normTeam, teamMatches, MARKET_RELIABILITY, settlementMetrics, normalMetrics, parseHomeHandicap, selectedHandicap };
