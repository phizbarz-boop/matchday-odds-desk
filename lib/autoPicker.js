...
function buildCandidates({ predictions, footballMarkets, basketballWinner, basketballTotals, hockeyWinner, hockeyTotals, handballWinner, handballTotals, minProbability = 55, minEdge = 0, leagues = null, sportScope = 'all', betTypes = null }) {
  ...
  const scope = ['all','football','basketball','hockey','handball'].includes(String(sportScope).toLowerCase()) ? String(sportScope).toLowerCase() : 'all';
  const candidates = [];
  ...
  for (const [sport,payload,scopeKey] of [['Basketball',basketballWinner,'basketball'],['Ice Hockey',hockeyWinner,'hockey'],['Handball',handballWinner,'handball']]) {
    ...
    for (const e of events) {
      ...
      const bt = scopeKey === 'basketball' ? 'basketball_winner' : (scopeKey === 'hockey' ? 'hockey_winner' : 'handball_winner');
  ...
  // Basketball, Ice Hockey and Handball totals. Each total line (e.g. 214.5 points or 5.5 goals)
  ...
  // the honest probability estimate is the market's margin-removed probability. Handball uses this exact same model.
  for (const [sport,payload,scopeKey] of [['Basketball',basketballTotals,'basketball'],['Ice Hockey',hockeyTotals,'hockey'],['Handball',handballTotals,'handball']]) {
    if (scope !== 'all' && scope !== scopeKey) continue;
