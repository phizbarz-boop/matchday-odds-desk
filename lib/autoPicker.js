...
function buildCandidates({ predictions, footballMarkets, basketballWinner, basketballTotals, hockeyWinner, hockeyTotals, handballWinner, handballTotals, volleyballWinner, volleyballTotals, volleyballSets, minProbability = 55, minEdge = 0, leagues = null, sportScope = 'all', betTypes = null }) {
  ...
  const scope = ['all','football','basketball','hockey','handball','volleyball'].includes(String(sportScope).toLowerCase()) ? String(sportScope).toLowerCase() : 'all';
  const candidates = [];
  ...
  for (const [sport,payload,scopeKey] of [['Basketball',basketballWinner,'basketball'],['Ice Hockey',hockeyWinner,'hockey'],['Handball',handballWinner,'handball'],['Volleyball',volleyballWinner,'volleyball']]) {
    ...
    for (const e of events) {
      ...
      const bt = scopeKey === 'basketball' ? 'basketball_winner' : (scopeKey === 'hockey' ? 'hockey_winner' : (scopeKey === 'handball' ? 'handball_winner' : 'volleyball_winner'));
  ...
  for (const [sport,payload,scopeKey] of [['Basketball',basketballTotals,'basketball'],['Ice Hockey',hockeyTotals,'hockey'],['Handball',handballTotals,'handball'],['Volleyball',volleyballTotals,'volleyball']]) {
    if (scope !== 'all' && scope !== scopeKey) continue;
