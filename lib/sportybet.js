...
<<<<<<< HEAD
  const callNg = async () => {
    // Keep the exact request contract that was working before the corner changes.
    // Parse's NG endpoint documents this HTTP body as a JSON-string field.
  ...
  const callFull = async () => {
    // Full-market endpoint is required for selections discovered there (corners/1UP).
    const payload = await parseFetch('book_bet', {
      method: 'POST',
...
>>>>>>> b755ab6 (Add H2H weighting and SportyBet integration)
    });
    return normalizeBookingResult(payload);
  };

<<<<<<< HEAD
  const order = preferFullMarket
    ? [['full-market', callFull], ['Nigeria', callNg]]
    : [['Nigeria', callNg], ['full-market', callFull]];

  for (const [label, fn] of order) {
    try {
      const result = await fn();
      if (hasBookingCode(result)) return result;
      attempts.push(`${label}: no booking code returned`);
    } catch (err) {
      attempts.push(`${label}: ${err.message}`);
      console.warn(`SportyBet ${label} book_bet failed: ${err.message}`);
    }
  }
