...
const FOOTBALL_MARKETS = {
  ...
  ou:    { query: 'Over/Under', marketId: '18', specifier: 'total=2.5' },
};
...
async function parseFetch(endpoint, { method = 'GET', params = {}, body } = {}) {
...
async function pagedMarket(endpoint, { market, marketId = null, specifier = null, hours = 96, sport = null } = {}) {
  const pageSize = Math.max(1, Math.min(100, parseInt(process.env.SPORTYBET_PAGE_SIZE || '100', 10)));
  const maxPages = Math.max(1, Math.min(20, parseInt(process.env.SPORTYBET_MAX_PAGES || '5', 10)));
  const collected = [];
  let total = null;
...
async function getFootballMarket(kind, { hours = 96 } = {}) {
  const cfg = FOOTBALL_MARKETS[kind];
  if (!cfg) throw new Error(`Unsupported SportyBet football market: ${kind}`);
  const normalized = await pagedMarket('get_prematch_football_markets', {
    market: cfg.query,
...
async function getSportMarket(sport, kind, { hours = 96 } = {}) {
...
module.exports = { FOOTBALL_MARKETS, SPORT_CONFIG, getFootballMarket, getSportMarket, bookBet, normalizeOutcome };
