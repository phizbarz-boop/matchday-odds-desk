...
const FOOTBALL_MARKETS = {
  ...
  // Full-time team corner totals. Keep separate from match-total corners so a team line
  // can never be misread as a match-total line.
  full_time_team_corners: { query: process.env.SPORTYBET_TEAM_CORNERS_MARKET_QUERY || 'Team Corners', marketId: null },
...
function isDetailedMarketRow(row, kind) {
  ...
  if (kind === 'full_time_team_corners') return !firstHalf && teamish;
  if (kind === 'corners') return !firstHalf && !teamish;
  return false;
}
...
async function getFootballMarket(kind, { hours = 96, maxPages = null } = {}) {
  ...
  if (kind === 'corners' || kind === 'full_time_team_corners' || kind === 'first_half_team_corners' || kind === 'oneup') {
