...
async function bookBet(selections) {
  ...
  // Corner/1UP selections are discovered from the subscribed full-market scraper
  // (BOOKING_BASE). Their event/market/outcome IDs may not exist in the older NG
  // scraper namespace, so booking must be attempted against the same scraper first.
  //
  // For compatibility with older standard-market selections, fall back to BASE if
  // the full-market booking endpoint rejects the request.
  const body = { selections: JSON.stringify(cleaned) };
  let firstErr = null;
  ...
  try {
    ...
    if (result && (result.shareCode || result.bookingCode || result.code || result.shareURL)) {
      return result;
    }
    // Some scraper responses return success without a usable code. Treat that as a
    // failed booking attempt so the compatibility fallback can run.
    firstErr = new Error('Full-market SportyBet booking endpoint returned no booking code');
  } catch (err) {
    firstErr = err;
    console.warn(`SportyBet full-market book_bet failed: ${err.message}`);
  }
  ...
  try {
    ...
    throw new Error('Legacy SportyBet booking endpoint returned no booking code');
  } catch (secondErr) {
    const err = new Error(
      `SportyBet booking failed on both market sources. Full-market: ${firstErr?.message || 'unknown'} | Legacy: ${secondErr.message}`
    );
