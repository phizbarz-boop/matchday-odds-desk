  ...
  if (kind === 'corners' || kind === 'first_half_team_corners' || kind === 'oneup') {
    ...
    if (normalized.length === 0) {
      try {
        ...
        // The current single Parse API may not expose legacy full-market endpoints
        // (get_upcoming_events/get_event_odds). Missing optional special markets must
        // not abort Telegram or Auto Builder; simply return zero rows for that market.
        console.warn(`[SportyBet] optional ${kind} fallback unavailable: ${err.message}`);
        normalized = [];
      }
    }
  }
