...
function isDetailedMarketRow(row, kind) {
  ...
  if (kind === 'oneup') {
    ...
    return /\b1up\b|1x2\s*[-–:]?\s*1up|lead\s+by\s+(?:1|one)\s+goal/.test(text);
  }
...
async function getDetailedFootballMarket(kind, { hours = 96, maxPages = null } = {}) {
  ...
  const pageLimit = Math.max(1, Math.min(10, parseInt(maxPages || process.env.SPORTYBET_DETAIL_MAX_PAGES || '2', 10)));
  const maxEvents = Math.max(1, Math.min(150, parseInt(process.env.SPORTYBET_DETAIL_MARKET_MAX_EVENTS || '60', 10)));
  const cutoff = Date.now() + Math.max(1, Number(hours || 96)) * 3600 * 1000;
  const upcoming = [];

  for (let page = 1; page <= pageLimit && upcoming.length < maxEvents; page++) {
    const payload = await parseFetch('get_upcoming_events', {
      params: { page, sport:'football', page_size:pageSize, today_only:false },
      base: BOOKING_BASE,
    });
    const { events, total } = extractUpcomingEvents(payload);
    for (const e of events) {
      const t = Number(e.start_time ?? e.startTime ?? 0);
      const ms = t && t < 1e12 ? t * 1000 : t;
      if (ms && ms > cutoff) continue;
      upcoming.push(e);
      if (upcoming.length >= maxEvents) break;
    }
    if (events.length < pageSize || upcoming.length >= total) break;
  }

  const rows = [];
  for (const e of upcoming.slice(0, maxEvents)) {
...
async function getFootballMarket(kind, { hours = 96, maxPages = null } = {}) {
  ...
  if ((kind === 'corners' || kind === 'first_half_team_corners' || kind === 'oneup') && normalized.length === 0) {
