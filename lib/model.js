...
function outcomeProbs(matrix) {
  let homeWin = 0, draw = 0, awayWin = 0, bttsYes = 0, homeOver05 = 0, awayOver05 = 0, homeUnder45 = 0, awayUnder45 = 0, over05 = 0, over15 = 0, over25 = 0, under45 = 0;
  const scores = [];
  for (let h = 0; h < matrix.length; h++) {
    for (let a = 0; a < matrix[h].length; a++) {
      const p = matrix[h][a];
      ...
      if (h > 0) homeOver05 += p;
      if (a > 0) awayOver05 += p;
      if (h < 5) homeUnder45 += p;
      if (a < 5) awayUnder45 += p;
      if (h + a > 0) over05 += p;
      if (h + a > 1) over15 += p;
      if (h + a > 2) over25 += p;
      if (h + a < 5) under45 += p;
  ...
  return {
    ...
    homeOver05, awayOver05, homeUnder45, awayUnder45,
    over05,
...
function summarizeH2H(matches, currentHome, currentAway, maxMeetings = 8, decay = 0.82) {
  ...
  if (!relevant.length) {
    return {
      ...
      bttsRate: null, homeOver05Rate: null, awayOver05Rate: null, homeUnder45Rate: null, awayUnder45Rate: null, over05Rate: null, over15Rate: null, over25Rate: null, under45Rate: null, weightedTotal: 0,
      samples: [],
    };
  }

  let wTotal = 0, hWins = 0, draws = 0, aWins = 0, btts = 0, homeOver05 = 0, awayOver05 = 0, homeUnder45 = 0, awayUnder45 = 0, over05 = 0, over15 = 0, over25 = 0, under45 = 0;
  const samples = [];
  relevant.forEach((m, idx) => {
    ...
    if (currentHomeGoals > 0) homeOver05 += w;
    if (currentAwayGoals > 0) awayOver05 += w;
    if (currentHomeGoals < 5) homeUnder45 += w;
    if (currentAwayGoals < 5) awayUnder45 += w;
    if (currentHomeGoals + currentAwayGoals > 0) over05 += w;
  ...
  return {
    ...
    homeOver05Rate: homeOver05 / wTotal,
    awayOver05Rate: awayOver05 / wTotal,
    homeUnder45Rate: homeUnder45 / wTotal,
    awayUnder45Rate: awayUnder45 / wTotal,
    over05Rate: over05 / wTotal,
...
function blendPredictionWithH2H(base, h2h, maxWeight = 0.18) {
  ...
  const homeOver05 = base.homeOver05 * keep + h2h.homeOver05Rate * weight;
  const awayOver05 = base.awayOver05 * keep + h2h.awayOver05Rate * weight;
  const homeUnder45 = base.homeUnder45 * keep + h2h.homeUnder45Rate * weight;
  const awayUnder45 = base.awayUnder45 * keep + h2h.awayUnder45Rate * weight;
  const over05 = base.over05 * keep + h2h.over05Rate * weight;
