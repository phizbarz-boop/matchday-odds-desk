...
function outcomeProbs(matrix) {
  let homeWin = 0, draw = 0, awayWin = 0, bttsYes = 0, over05 = 0, over15 = 0, over25 = 0, under45 = 0;
  const scores = [];
  for (let h = 0; h < matrix.length; h++) {
    for (let a = 0; a < matrix[h].length; a++) {
      const p = matrix[h][a];
...
function summarizeH2H(matches, currentHome, currentAway, maxMeetings = 8, decay = 0.82) {
  ...
  if (!relevant.length) {
    return {
      ...
      bttsRate: null, over05Rate: null, over15Rate: null, over25Rate: null, under45Rate: null, weightedTotal: 0,
      samples: [],
    };
  }

  let wTotal = 0, hWins = 0, draws = 0, aWins = 0, btts = 0, over05 = 0, over15 = 0, over25 = 0, under45 = 0;
  const samples = [];
